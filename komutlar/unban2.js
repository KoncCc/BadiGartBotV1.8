...
module.exports.run = async (client, message, args) => {
  ...
  const userError = new Discord.MessageEmbed()
    .setColor('#ed455a')
      .setTitle('• Hata: 02 •')
        .setDescription('```Yasağı kaldırmak için bir kullanıcı kimliği girmelisiniz bg!unban İD```')
  ...
  if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send
        (permError).then
          (message.delete()).then
            (msg => msg.delete({timeout: 11000, reason: "xD"}));
 ...
 // console.log(banList.map(s => s.users.id))
  ...
  if (!user.id === banList) return message.channel.send
      (userError3).then
        (message.delete()).then
          (msg => msg.delete({timeout: 15000, reason: "xD"}));
